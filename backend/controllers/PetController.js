...
module.exports = class PetController {
    ...
    static async removePetById(req, res){
       ...
       // check if pet exists
        ...
        // check if pet was register by user
        const token = getToken(req)
        const user = await getUserByToken(token)
    ...
    static async schedule(req, res){
        ...
        res.status(200).json({message: `A visita foi agendada com sucesso, entre em contrado com ${pet.user.name} pelo telefone ${pet.user.phone}`})
    ...
    static async concludeAdoption(req, res) {
        ...
        res.status(200).json({message: `Parabéns! O ciclo de adoção foi finalizado`})
